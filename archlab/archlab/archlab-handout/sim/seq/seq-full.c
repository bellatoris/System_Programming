...
int gen_instr_valid()
{
    return ((icode) == (I_NOP) || (icode) == (I_HALT) || (icode) == 
      ...
      (icode) == (I_MRMOVL) || (icode) == (I_LEAVE) || (icode) == (I_IADDL)
...
int gen_srcA()
{
    return (((icode) == (I_RRMOVL) || (icode) == (I_RMMOVL) || (icode) == 
        (I_ALU) || (icode) == (I_PUSHL)) ? (ra) : ((icode) == (I_LEAVE)) ? 
      (REG_EBP) : ((icode) == (I_POPL) || (icode) == (I_RET)) ? (REG_ESP)
       : (REG_NONE));
}
...
int gen_aluB()
{
    return (((icode) == (I_RMMOVL) || (icode) == (I_MRMOVL) || (icode) == 
        (I_ALU) || (icode) == (I_CALL) || (icode) == (I_PUSHL) || (icode)
         == (I_RET) || (icode) == (I_POPL) || (icode) == (I_LEAVE) || 
        (icode) == (I_IADDL)) ? (valb) : ((icode) == (I_RRMOVL) || (icode)
         == (I_IRMOVL)) ? 0 : 0);
}
